...
Import-Module (Join-Path $PSScriptRoot "..\..\..\Modules\WorkflowManagerDSC\Modules\WorkflowManagerDSC.Util\WorkflowManagerDSC.Util.psm1" -Resolve)

InModuleScope "WorkflowManagerDSC.Util" {
    Describe "WorkflowManagerDSC.Util tests [WAC server version $((Get-Item $Global:CurrentWACCmdletModule).Directory.BaseName)]" {
        ...
        Context "Validate Test-OosDscParameterState" {
            It "Returns true for two identical tables" {
                ...
                Test-WFDscParameterState -CurrentValues $desired -DesiredValues $desired | Should Be $true
            }

            It "Returns false when a value is different" {
                $current = @{ Example = "something" }
                $desired = @{ Example = "test" }
                Test-WFDscParameterState -CurrentValues $current -DesiredValues $desired | Should Be $false
            }
            ...
            It "Returns true when only a specified value matches, but other non-listed values do not" {
                ...
                Test-WFDscParameterState -CurrentValues $current -DesiredValues $desired -ValuesToCheck @("Example") | Should Be $true
            }

            It "Returns false when only specified values do not match, but other non-listed values do " {
                $current = @{ Example = "test"; SecondExample = "true" }
                $desired = @{ Example = "test"; SecondExample = "false"  }
                Test-WFDscParameterState -CurrentValues $current -DesiredValues $desired -ValuesToCheck @("SecondExample") | Should Be $false
            }
